...
function initRuntimeSocketService() {
  const hosts = "26.26.26.1,192.168.1.36,127.0.0.1,172.29.224.1";
  const port = "8090";
  const id = "mp-weixin_9wlzBS";
  ...
  _prepare() {
    ...
    for (var u2 = 1; u2 < e2; u2++)
      for (var h2 = 0; h2 < 4; h2++)
        o2 = this._Kd[u2][h2], this._Kd[u2][h2] = En[o2 >> 24 & 255] ^ kn[o2 >> 16 & 255] ^ An[o2 >> 8 & 255] ^ Pn[255 & o2];
  }
  encrypt(e2) {
    if (16 != e2.length)
...
class $n extends sn {
  ...
  async getUserEncryptKey() {
    ...
    return new Promise((e2, t2) => {
      index.getUserCryptoManager().getLatestUserKey({ success: (t3) => {
        ...
        t2(nn({ ...en.GET_ENCRYPT_KEY_FAILED, cause: e3 }));
      } });
    });
  }
  getWxAppId() {
  ...
  async platformGetSignOption() {
    ...
    return { verifyClientSign: qn(JSON.stringify({ data: JSON.stringify({}), appId: this.appId, deviceId: this.deviceId, wxAppId: this.getWxAppId(), simulator: "devtools" === ce().platform, timestamp: Date.now() }), e2, t2), encryptKeyId: n2, mpWeixinCode: this.scopedGlobalCache.mpWeixinCode, mpWeixinOpenid: this.scopedGlobalCache.mpWeixinOpenid };
  }
  async platformEncryptData(e2) {
    ...
    return this.secretType === Yt.RESPONSE ? { content: e2, _uniCloudOptions: r2 } : { content: qn(JSON.stringify({ data: JSON.stringify(e2), appId: this.appId, deviceId: this.deviceId, wxAppId: this.getWxAppId(), simulator: "devtools" === ce().platform, timestamp: Date.now() }), t2, n2), _uniCloudOptions: r2 };
  }
