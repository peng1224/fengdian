  ...
  setup(__props) {
    ...
    const handleChooseLocation = () => {
      common_vendor.index.chooseLocation({
        ...
        fail: (err) => {
          common_vendor.index.__f__("error", "at pages/appointmentForm/appointmentForm.vue:172", "选择地点失败", err);
          if (err.errMsg === "chooseLocation:fail cancel" || err.errMsg === "chooseLocation:fail") {
            common_vendor.index.showToast({ title: "已取消选点", icon: "none" });
          } else {
            common_vendor.index.showToast({ title: "选点失败: " + err.errMsg, icon: "none" });
          }
        }
      });
    };
    ...
    const fetchBookedSlots = async (date) => {
      ...
      try {
        ...
        common_vendor.index.__f__("error", "at pages/appointmentForm/appointmentForm.vue:245", "获取预约时段失败:", error);
      ...
      try {
        ...
        const res = await common_vendor.tr.callFunction({
          ...
          data: {
            ...
            total_fee,
            // 使用定义的 total_fee
            clientInfo: ((_a = getApp().globalData) == null ? void 0 : _a.clientInfo) || common_vendor.index.getSystemInfoSync()
          }
        });
        ...
        if (err && err.errMsg && err.errMsg.includes("requestPayment:fail")) {
          ...
          common_vendor.index.__f__("error", "at pages/appointmentForm/appointmentForm.vue:359", "submitForm error:", err);
        }
      }
    };
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        b: common_vendor.f(availableDays.value, (day, index, i0) => {
          return {
            ...
            e: common_vendor.o(($event) => selectDate(day.dateString), index)
          };
        }),
        c: isLoading.value
        ...
        e: common_vendor.f(timeSlots.value, (slot, index, i0) => {
          return {
            ...
            d: common_vendor.o(($event) => selectTime(slot), index)
          };
        })
      }, {
